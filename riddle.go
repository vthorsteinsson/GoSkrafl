...
var IcelandicHeuristics = HeuristicConfig{
	MinTiles:       40,
	MaxTiles:       60,
	MinMoves:       20,
	MaxMoves:       300,
	...
	BingoBonus:     50.0,
	ScoreGapBonus:  1.5,
	SolutionFilter: IcelandicCommonWordsDictionary,
}
...
func generateCandidate(ctx context.Context, params GenerationParams, heuristics HeuristicConfig) (*RiddleCandidate, error) {
	...
	if numMoves < heuristics.MinMoves || numMoves > heuristics.MaxMoves {
	...
	if heuristics.SolutionFilter != nil {
		if !heuristics.SolutionFilter.Find(tm.CleanWord()) {
...
func GenerateRiddle(params GenerationParams, heuristics HeuristicConfig) (*Riddle, error) {
		...
		go func() {
			...
			for {
				...
				case <-ctx.Done():
					return
				default:
					candidate, err := generateCandidate(ctx, params, heuristics)
	...
	if len(bestCandidates) == 0 {
		return nil, fmt.Errorf("could not generate a suitable riddle in the allotted time")
	}
	...
	return bestCandidates[0].Riddle, nil
}
