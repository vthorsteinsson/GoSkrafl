...
### Coordinate Format in RiddleJSON

Within the `RiddleJSON` field, the `solution.coord` follows a specific format based on the move's orientation:
- **Horizontal Moves**: `RowLetter + ColumnNumber` (e.g., "A1", "H8", "O15")
- **Vertical Moves**: `ColumnNumber + RowLetter` (e.g., "1A", "8H", "15O")

This convention ensures consistency with the `solution.description` field and is correctly generated by the `TileMove.Coordinate()` method.
